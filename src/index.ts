...
const startWebServer = (client: ExtendedClient, rest: REST) => {
	...
	app.post(
		"/api/guilds/:id/earthquake-webhook",
		express.json(),
		async (req: Request<{ id: string }>, res: Response) => {
			const guildId = req.params.id;
			...
			try {
				const createWebhook = async (c: Client, gId: string, cId: string) => {
				...
				if (!client.shard) {
					const webhookUrl = await createWebhook(client, guildId, channelId);
					if (!webhookUrl) {
						return res
							.status(404)
							.json({ error: "Guild, channel, or permission not found" });
					}
				...
				console.error("Earthquake webhook error:", err);
				return res.status(500).json({ error: "Internal server error" });
			}
		},
	);
